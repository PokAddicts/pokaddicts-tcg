...
create table if not exists cards (
  ...
  market_value_sgd numeric, -- cached SGD price, refreshed daily by the refresh-catalog-prices Edge Function (English cards only) - the single "best" figure (TCGPlayer preferred), for anywhere only one number is shown
  ...
  tcgdex_variants jsonb, -- cached array of {label, price, source} - the FULL TCGPlayer (every sub-variant: Normal/Holofoil/Reverse Holofoil/1st Edition/etc.) + CardMarket breakdown, same shape as pokewallet_variants/snkrdunk_conditions use for Japanese cards. TCGdex itself already returns both sources with no rate limit (confirmed directly in code - PokeWallet was never actually needed for English pricing, only Japanese, where TCGdex's own numbers were found unreliable), this just caches the full breakdown instead of collapsing it to one number the way market_value_sgd does. Written by the same refresh-catalog-prices cron.
